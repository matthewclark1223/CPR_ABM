...
abm<-function(#Specified parameters
  ...
  startData<-agents
  startData$t<-rep(1,nrow(startData))
  AllData<-list()
  ...
  AllData<-rbind(startData,AllData)
  AllData$t<-as.integer(AllData$t)
...
library("ggimage")
AllData$Image<-rep("https://jeroenooms.github.io/images/frink.png",nrow(AllData))
AllData%>%filter(t==3)%>%
ggplot(.,aes(x=PercTimeProtected,y=PayoffTotalLastTime))+ 
