...
img.class <- mask(raster("pemmyLC2018updateSAR.tif"), pemba)
#img.class <- mask(raster("pemmyLC2018only.tif"), pemba)
val2018 <- st_read('points2018stratSAR.shp')

## create regular accuracy matrix 
#confmat <- table(as.factor(val2018$classifica), as.factor(val2018$Class))
#confmat ## 99 CLOUDS one each 5 and 6

val2018$map <- extract(img.class, val2018)
# create regular accuracy matrix 
confmat <- table(as.factor(val2018$classifica), as.factor(val2018$Class))
#confmat <- table(as.factor(val2018$map), as.factor(val2018$Class))
...
nclass <- length(unique(val2018$classifica))
## class labels,
classes <- seq(0,7,1)
...
print(xresult, file = '../output/areaacc2018.html')
print(xresult, file = '../output/areaacc2018.txt')
...
print(xconfmat, file = '../output/confmat2018.html')
print(xconfmat, file = '../output/confmat2018.txt')
