...
####Just one area
map <- get_googlemap("mjimbini tanzania", scale=2, zoom=14, maptype = "hybrid")
#map <- get_map("mjimbini tanzania", scale=2,zoom=14,maptype = "terrain",source="stamen" )
map <- ggmap_bbox(map)
ggmap(map) + 
  coord_sf(crs = st_crs(3857)) + # force the ggplot2 map to be in 3857
  geom_sf(data = filter(Pemba_3857,NAME_3 =="Mjimbini"), 
          color="yellow",size=2,fill=NA, inherit.aes = FALSE)+
...
ggplot()+
  geom_sf(data = filter(Pemba_3857,NAME_3 =="Kangani"), fill=NA, inherit.aes = FALSE)
