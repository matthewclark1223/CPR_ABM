...
PCC = (10000*0.2) #Protected carrying capacity
sample_r<-(SucBiasdat[1:sample_days,]$NumResourcesProtect)
UpCC<-(10000*0.8) #Unprotected CC from ABM
sample_UP_r<-(SucBiasdat[1:sample_days,]$NumResourcesWorking)
...
stan_d = list(n_obs = sample_days,
              ...
              PCC = PCC,
              UpCC = UpCC,
              S0 = 0.98, #starting susceptible from ABM
              PAR0 = 0.2, #starting CC from ABM
              Up0 = 0.2, #starting CC from ABM
              PaR = sample_r,
              UpR = sample_UP_r,
              ts = sample_time,
              fake_ts = c(1:t_max))
