                       ...
                       tabPanel("AbmOutput",
                                ...
                                fluidRow(sidebarPanel(width = 3,
                                                      ...
                                                      sliderInput("percProtect",
                                                                  ...
                                                                  max = 0.99,
                                                                  value = 0.2),
                                                      sliderInput("enrolledStart",
                                                                  "Percent of population enrolled at start",
                                                                  min = 0.0,
                                                                  max = 1.0,
                                                                  value = 0.2),
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    enrolledStart<-input$enrolledStart
    ...
    abm(Individuals = individuals, TotalCarryingCapacity =TotalCarryingCapacity, 
        PercProtected=percProtect, EnrollPercStart=enrolledStart,
  ...
  output$data <- DT::renderDataTable({
    ...
    modResult<-abm(Individuals = individuals, TotalCarryingCapacity =TotalCarryingCapacity, 
                   PercProtected=percProtect, EnrollPercStart=enrolledStart,
                   ResourceRegenerationPerTimeStep=resourceRegen,harvestMax= maxHarvest,
                   TimeSteps=timeSteps,StartPercCarryingCapacity=StartPercCarryingCapacity,
                   ProbOfMobility=ProbOfMobility,LearningStrategy = LearningStrategy )
    
    DT::datatable(modResult, extensions = c('Buttons','Scroller') , options = list(dom='Bfrtip',buttons=c('copy', 'csv', 'excel', 'pdf', 'print'), deferRender = TRUE,
