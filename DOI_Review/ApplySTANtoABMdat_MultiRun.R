source("Binary_Adoption_ABM_MultiRun.R")
...
sample_n<-100
t_max<-100
sample_y<-ConformBiasdat[1:sample_days,]$Enrolled
...
#cor(mod_median[-(1:sample_days)],(ConformBiasdat[(sample_days+1):sample_n,]$Enrolled)/sample_n)
pred<-mod_median[-(1:sample_days)]
obs<-(ConformBiasdat[(sample_days+1):sample_n,]$Enrolled)/sample_n
...
ggplot(df_sample, aes(x=sample_time, y=sample_prop)) +
  ...
  geom_point(data=ConformBiasdat[sample_days+1:sample_n,],aes(x=timeStep,y=Enrolled/100,fill="Test"),
  ...
  annotate("text", x = 70, y = 0.6, label = paste0("MAE =", round((Metrics::mae(pred,obs)*100),2),"%"),
           fontface="bold" )+
...
ggplot(df_sample, aes(x=sample_time, y=sample_prop)) +
  ...
  annotate("text", x = 70, y = 0.9, label = paste0("MAE =", round((Metrics::mae(pred,obs)*100),2),"%"),
           fontface="bold" )+
