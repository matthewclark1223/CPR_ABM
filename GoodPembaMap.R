...
#Two equal sized maps with reduced margins in between
...
#alternative method. Inset map
library(grid)
Pemba <- st_crop(Tanzania, xmin = 39.1, xmax = 39.9,
                 ymin = -5.6, ymax = -4.9)
...
Pem_fin<-ggplot() + geom_sf(data = Pemba, fill="antiquewhite")+coord_sf(xlim = c(39.36,39.86))+ theme_bw()+
  ...
  annotation_scale(location = "bl", width_hint = 0.5) +theme(plot.margin=unit(c(0.5,0,0.5,0),"cm"))

EastAfrica <- st_crop(africamap, xmin = 32, xmax = 60,
                      ymin = -18, ymax = 2)
EAfrFin<-ggplot() + geom_sf(data = EastAfrica, fill="antiquewhite")+coord_sf(expand = FALSE)+ theme_bw()+
  geom_rect(aes(xmin=39.5, xmax=40.1, ymin=-5.7, ymax=-4.5),fill="#b2df8a", color="#33a02c", alpha=0.5)+
  annotate(geom = "text", x = 38, y = 0.8, label = "Kenya", fontface = "italic", color = "grey22", size = 3.5)+
  annotate(geom = "text", x = 35, y = -6, label = "Tanzania", fontface = "italic", color = "grey22", size = 3.5)+
  annotate(geom = "text", x = 47, y = -16.5, label = "Madagascar", fontface = "italic", color = "grey22", size = 3.5)+
  annotate(geom = "text", x = 37.5, y = -13, label = "Mozambique", fontface = "italic", color = "grey22", size = 3.5)+
  theme( panel.background = element_rect(fill = "#c6dbef"),panel.grid.major = element_blank(),
         axis.text=element_blank(), axis.ticks = element_blank())+
  annotation_scale(location = "bl", width_hint = 0.5) +ylab("")+xlab("")+
  theme(plot.margin = unit(c(0,0,-2,-0.5), unit = "in"))
...
#Make the map
Pem_fin
print(EAfrFin, vp = viewport(0.382, 0.886, width = 0.2, height = 0.2))
