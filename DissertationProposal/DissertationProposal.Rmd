...
With all model parameters kept constant as in the output shown in (fig \@ref(fig:abm3)), we can run the model with just 5% of the agents assigned as having the possibility to defect at initialization and all agents modifying harvest strategies via success-biased learning. The average final time step outcomes from five runs of this model can be seen in (fig \@ref(fig:abm4)).
...
```
Allowing agents to modify harvest strategies and harvest from the protected area greatly reduces the overall harvest outcomes. To better understand the underlying dynamics, we set the starting model parameters to the ideal conditions for maximum payoff as identified in (fig \@ref(fig:abm3)) and observe the model behavior at each time step.
...
kbl(abmVars,format="latex", caption = "Description of all model variables and outcomes",escape=F) %>% 
