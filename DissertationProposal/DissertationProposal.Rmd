...
Inline LaTeX equations can be written in a pair of dollar signs using the LaTeX syntax, e.g., $f(k) = {n \choose k} p^{k} (1-p)^{n-k}$  math expressions of the display style can be written in a pair of double dollar signs, e.g., 

$$f(k) = {n \choose k} p^{k} (1-p)^{n-k}$$, and the output looks like this:


Cultural fst equation
...
Price equation
$$ \bar{W} \Delta \bar{Z} = \text{cov}(W_{g},Z_{g}) + E[\text{cov}(W_{ig},Z_{ig})]$$
